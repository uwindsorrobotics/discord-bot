...
def convert(word):
    ...
    for i in range(len(word)):
        if doubleT or th_Presence:
            ...
            converted += (("D" if word[i].isupper() else "d") + ("D" if word[i + 1].isupper() else "d")) if i + 1 < len(
                word) else "t"
            ...
            converted += word[i]
    if word[0] != ":" or word[-1] != ":":
        convertedSentence += ((converted[0] + "-" + converted[0:]) if (
                rand(1, 10) == 1 and converted[0] in alphabet) else converted) + " "
    else:
